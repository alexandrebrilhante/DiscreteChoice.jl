...
# Gradient descent.
@test optimize(f, g, zeros(2), GradientDescent(), 100000)[1] ≈ ones(2)

# Newton.
@test optimize(f, g, H, zeros(2), Newton())[1] ≈ ones(2)
...
@test optimize(f, g, H, tcg, zeros(2), TrustRegion())[1] ≈ ones(2)

# Trust region using Steihaug-Toint's method and approximative Hessian using BFGS.
@test optimize(f, g, BFGS!, tcg, zeros(2), TrustRegion(), true)[1] ≈ ones(2)

# Trust region using Steihaug-Toint's method and approximative Hessian using SR1.
@test optimize(f, g, SR1!, tcg, zeros(2), TrustRegion(), true)[1] ≈ ones(2)
